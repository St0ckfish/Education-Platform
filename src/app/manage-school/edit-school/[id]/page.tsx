...
import { useGetCurriculumQuery, useGetEducationsQuery, useGetLanguagesQuery, useGetRegionQuery, useGetTypeQuery } from "@/app/add-new-school/api/createSchoolApi";
import { useGetSchoolByIdQuery, useUpdateSchoolMutation } from "../../api/manageSchool";
...
const EditSchool = () => {
    ...
    const { isSuccess: successSchool, data: dataSchool } = useGetSchoolByIdQuery({ token, id: params.id })
    const [updateSchool, { error, isError, isSuccess, isLoading }] = useUpdateSchoolMutation()
    ...
    useEffect(() => {
        if (successSchool && dataSchool?.data) {
            ...
            setFallSemesterStartDate(dataSchool.data.semesterDate?.fallSemesterStartDate || "");
            setFallSemesterEndDate(dataSchool.data.semesterDate?.fallSemesterEndDate || "");
            setSpringSemesterStartDate(dataSchool.data.semesterDate?.springSemesterStartDate || "");
            setSpringSemesterEndDate(dataSchool.data.semesterDate?.springSemesterEndDate || "");
            setSummerSemesterStartDate(dataSchool.data.semesterDate?.summerSemesterStartDate || "");
            setSummerSemesterEndDate(dataSchool.data.semesterDate?.summerSemesterEndDate || "");
            ...
            setNumberOfLegalAbsenceDays(dataSchool.data.numberOfLegalAbsenceDays?.toString() || "");
        }
        ...
        <>
            {successSchool && (
                <div className="lg:ml-[270px] mr-[5px] grid justify-center items-center mt-10">
                    <form>
                        <h1 className="font-bold text-[28px] mb-4 font-sans text-[#041631] dark:text-white">Update School</h1>
                        ...
                        <div className="grid p-10 bg-white card rounded-xl items-center justify-center xl:w-[1000px] lg:w-[750px]  gap-5 md:w-[600px] sm:w-[500px]">
                            ...
                            <div className="flex justify-center text-center">
                                {isLoading ? (
                                    ...
                                    <button onClick={(e) => handleSend(e)} type="submit" className="px-4 py-2 rounded-xl bg-[#3E5AF0] hover:bg-[#4a5cc5] hover:shadow-xl text-white text-[18px] w-[160px] ease-in duration-300">Update school</button>
