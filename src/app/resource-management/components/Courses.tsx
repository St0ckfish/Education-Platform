...
const Courses: React.FC<CoursesProps> = ({
  ...
  const handleDelete = async (): Promise<void> => {
    if (selectedCourseId !== null) {
      ...
      try {
        const response: DeleteCourseResponse = await deleteCourse({
          token,
          id: selectedCourseId,
        });
  ...
  return (
    <>
      {isLoading ? (
        ...
        <div className="relative px-2 mt-5 overflow-x-auto bg-transparent sm:rounded-lg max-[1200px]:w-screen h-screen">
          ...
          <div className="flex justify-between max-[502px]:grid max-[502px]:justify-center text-center">
            ...
            <div className="flex justify-center">
              <Link
                href="/create-course"
                className="px-4 py-2 whitespace-nowrap rounded-xl bg-[#3E5AF0] hover:bg-[#4a5cc5] hover:shadow-xl mb-5 mr-3 text-white text-[18px] w-[180px] ease-in font-semibold duration-300"
              >
                + Add new course
              </Link>
            </div>
            ...
            <table className="w-full overflow-x-auto text-sm text-left rtl:text-right text-gray-500 ">
              ...
              <tbody>
                {data?.data?.content?.map((item: any) => (
                  <tr
                    key={item.id}
                    className="bg-white border-b card  hover:bg-gray-50"
                  >
                    <th
                      scope="row"
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Link
                        href={`/resource-management/${item.id}`}
      ...
      >
        ...
        <Modal.Body>
          <div className="text-center">
            <h3 className="mb-5 z-[2000] text-lg font-normal text-gray-500 dark:text-gray-400">
